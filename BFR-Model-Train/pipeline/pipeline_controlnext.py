...
from models.controlnet_sdrm import ControlNetModel
...
class StableDiffusionXLControlNeXtPipeline(
    ...
    def prepare_image(
        ...
        image=image.to(dtype=self.torch_dtype)
    ...
    def postprocess_image(self, image):
        img_max,img_min=image.max(),abs(image.min())
        # deviding by img_max if the pixel values are greater than 1
        image[image>1]/=img_max
        # deviding by img_min if the pixel values are less than -1
        image[image<-1]/=img_min
        return image
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if XLA_AVAILABLE:
                    xm.mark_step()
        # import numpy as np
        # # visualize the controlnet output feature map, 1*3 plot
        # from PIL import Image
        # for j in range(3):
        #     plt.figure(figsize=(5, 5))
        #     f_map=controls['out'][j].detach().cpu().numpy()[0,:3,:,:].transpose(1,2,0)
        #     f_map=(f_map-f_map.min())/(f_map.max()-f_map.min())
        #     f_map.astype(np.float64)
        #     f_map=Image.fromarray((f_map*255).astype(np.uint8))
        #     plt.title(f'control_branch_{j+1}')
        #     plt.imshow(f_map)
        #     plt.savefig(f'tmp/feature_map/control_feature_{j+1}.png',dpi=300)
        #     plt.close()
        # plt.figure(figsize=(5, 5))
        # f_map=latents.detach().cpu().numpy()[0,:3,:,:].transpose(1,2,0)
        # # to [0,1]
        # f_map=(f_map-f_map.min())/(f_map.max()-f_map.min())
        # f_map.astype(np.float64)        
        # plt.title(f'unet_output')
        # plt.imshow(latents.detach().cpu().numpy()[0,:3,:,:].transpose(1,2,0))
        # plt.savefig('tmp/feature_map/unet_feature.png',dpi=300)
        # plt.close()
        if not output_type == "latent":
            # make sure the VAE is in float32 mode, as it overflows in float16
            needs_upcasting = self.vae.dtype == torch.float16 and self.vae.config.force_upcast
            # print(f"needs_upcasting: {needs_upcasting}")
            # print(f"vae dtype: {self.vae.dtype}, latents dtype: {latents.dtype}")
            # print(f"latent max: {latents.max()}, latent min: {latents.min()}")
            
            # latents_mean=latents.mean(dim=(2,3), keepdim=True)
            # latents_std=latents.std(dim=(2,3), keepdim=True)
            # controlnet_image_mean=controlnet_image.mean(dim=(2,3), keepdim=True)
            # controlnet_image_std=controlnet_image.std(dim=(2,3), keepdim=True)
            # latents=(latents-latents_mean)/(latents_std+1e-6)*controlnet_image_std+controlnet_image_mean
            if needs_upcasting:
                self.upcast_vae()
