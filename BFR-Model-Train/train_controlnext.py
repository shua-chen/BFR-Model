...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        validation_image_mean=(validation_image.mean()/validation_image.mean(axis=(0,1)))
        validation_image_gray_world=(validation_image *validation_image_mean ).clip(0,255).astype(np.uint8)
        
        validation_image_gray_world = Image.fromarray(validation_image_gray_world).convert("RGB")
        ...
        for _ in range(args.num_validation_images):
            with autocast_ctx:
                image = pipeline(
                    ...
                    controlnet_image=validation_image_gray_world,
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--generate_prompt",
        type=str,
        default="A high resolution natural human face image",
...
def gfpgan_degra(lq_imgs,restorer):
    # down sampling
    lq_imgs=F.interpolate(lq_imgs,scale_factor=0.5,mode="bilinear")
    restored_img = restorer.enhance(
            lq_imgs)
    # up sampling
    restored_img=F.interpolate(restored_img,scale_factor=2,mode="bilinear")
    return restored_img
...
def get_train_dataset(args, accelerator):
    ...
    while True:
        try:
            ...
            train_dataset =load_dataset("imagefolder",data_dir=args.train_data_dir,drop_labels=True,split="train")
...
def prepare_train_dataset(dataset, accelerator,opt,sr_dataset=None):
    ...
    def degrad(img):
        ...
        if img.size[0]>args.resolution:
            img = transforms.RandomCrop(args.resolution)(img)
        ...
        img_gt=((img_gt * (img_gt.mean() / 
                      img_gt.mean(axis=(0,1)))).
                      clip(0,255))
    ...
    def process_dataset(examples):
        ...
        if sr_dataset is not None:
            sr_images1=[degrad(exam_image) for exam_image in sr_dataset[args.image_column]]
            sr_images2=[degrad(exam_image) for exam_image in sr_dataset[args.image_column]]
            sr_images3=[degrad(exam_image) for exam_image in sr_dataset[args.image_column]]
            sr_images=sr_images1+sr_images2+sr_images3
            face_image_size=len(sr_images)
            # sample face_image_size images from new_images
            idx=np.random.choice(len(new_images), face_image_size, replace=False)
            new_images=new_images[idx]
            new_images=new_images+sr_images
        # list2tensor
...
def main(args):
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            load_safetensors(controlnet, os.path.join(path,'controlnet.safetensors'))
            load_safetensors(unet, os.path.join(path,'unet_weight_increasements.safetensors'), strict=False, load_weight_increasement=True)
    ...
    #logger.info(f"Number of trainable parameters in vae: {sum(p.numel() for p in vae_cp.parameters() if p.requires_grad)}")
    global_step = 0
    first_epoch = 0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, controlnet):
                ...
                # controls['scale'] *= min((args.controlnet_scale_factor*(global_step/500)),args.controlnet_scale_factor)
                ...
                pdm_loss=F.l1_loss(latents.float(), controls['sd_sample'].float(), reduction="mean")
                reconstruct_loss = F.l1_loss(model_pred.float(), target.float(), reduction="mean")
                loss = reconstruct_loss+pdm_loss
            ...
            pdm_loss = pdm_loss.detach().item()
            reconstruct_loss = reconstruct_loss.detach().item()
            ...
            logs = {"loss/step": loss,
                    'reconstruct_loss/step': reconstruct_loss,
                    'pdm_loss/step': pdm_loss,
                    'loss_avr/step': loss_avr, 'loss_ema/step': loss_ema, 'lr/step': lr_scheduler.get_last_lr()[0]}
